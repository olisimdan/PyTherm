...
cpa_consts = {}
for row in cpa_consts_sheet.iter_rows(min_row=3, min_col=1, max_col=20):
...
for const_id in cpa_consts.keys():
    ...
    cpa_parms = cpa_phys + [cpa_assoc_vol]
    ...
    # Define the association energy from SAPT.
    assoc_energy = [cpa_assoc_energy] * len(type)

    # Define an args list that we can pass to the least squares optimizer.
    cpa_sat_args = list(zip(assoc_energy, assoc_scheme, type))
    ...
    for i in temp_type_value:
        ...
        prop_type = i[1]
        prop_value = i[2]
        prop_calc_value = cpa_sat(prop_temp, *nm_theta, cpa_assoc_energy, cpa_scheme, prop_type)
        if prop_type == 'Saturation Pressure':
    ...
    results_sheet.append([const_id, 'DIPPR', *nm_theta, cpa_assoc_energy, pct_abs_dev_lden, pct_abs_dev_psat])
