...
def nelder_mead(x0, func, args=None, kwargs=None, bounds=None, constraints=None, small_tol=10.0**-10,
                flat_tol=10.0**-9, max_iter=10000, max_bisect_iter=100, initial_size=0.01):
    ...
    counter = 0
    while _infinity_check(f_simplex) and counter <= max_bisect_iter:
    ...
    if VERBOSE:
        ...
        print("iteration_count|small|flat|best_point")
    ...
    while small > small_tol and flat > flat_tol and counter < max_iter:
        ...
        ordered = np.argsort(f_simplex)
        if not _infinity_check(f_simplex):
            flat = np.absolute(f_simplex[ordered[-1]] - f_simplex[ordered[0]])
            small = np.linalg.norm(simplex[ordered[-1]] - simplex[ordered[0]])
            if VERBOSE:
                print(str(counter)+"|"+str(small)+"|"+str(flat)+"|"+str(simplex[ordered[0]]))
        counter = counter + 1
    return simplex[ordered[0]]


def particle_swarm(func, args=None, kwargs=None, bounds=None, constraints=None, small_tol=10.0**-10, flat_tol=10.0**-9,
    ...
    if VERBOSE:
        print("Particle Swarm Algorithm")
        print("iteration_count|small|flat|best_point")
    ...
    while counter < max_iter:
        ...
        if VERBOSE:
            print(str(counter)+"|"+str(small)+"|"+str(flat)+"|"+str(personal_best_position[final_ordered[0]]))
